"""Upgrade mv_coverage to schema version 9 — exclude type-V variation containers.

Changes vs v8:
- non_kit CTE now also excludes products with type = 'V' (variation containers).
  Type-V products are parent records that group their child variants (type S)
  in Tiny. They carry no own stock or sales — all data lives on the children.
  Showing V products was producing ghost rows (e.g. RON-COLL appearing as
  'discontinue' alongside its child RON-COLL-PRE which is 'rupture').
  36 V-type products removed; 247 S-type products remain.

Revision ID: mv_coverage_v9
Revises: mv_coverage_v8
...
revision = "mv_coverage_v9"
down_revision = "mv_coverage_v8"
branch_labels = None
depends_on = None
...
WITH non_kit AS (
    SELECT
        ...
        COALESCE(NULLIF(sup.supplier_name, ''), pfx_sup.supplier_name) AS supplier_name,
        sup.supplier_code
    FROM products p
    LEFT JOIN LATERAL (
...
_COMMENT = (
    ...
    "'schema_version=9; consumers: coverage-v3.sql, route.ts; "
