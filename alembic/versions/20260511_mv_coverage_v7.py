"""Upgrade mv_coverage to schema version 7 — clearance minimum sample size.

Changes vs v6:
- CLEARANCE rule now requires sold_30d_direct >= 3 before comparing prices.
  With fewer than 3 direct sales in the window the average price is statistically
  unreliable: a single sale at a promotional or erroneous price was triggering
  clearance for products that are genuinely healthy (e.g. RTA-GAV4-P with 2761
  kit-expansion sales and exactly 1 direct sale at R$15.57 vs prior R$19.72).
  Known false positives fixed by this threshold:
    - RTA-GAV4-P (1 direct sale in 30d, 2761 total) → pending
    - PRE-PT-CP-EFLEX-DARK (1 direct sale in 30d, stock=0) → rupture (active rule)

Revision ID: mv_coverage_v7
Revises: mv_coverage_v6
...
revision = "mv_coverage_v7"
down_revision = "mv_coverage_v6"
branch_labels = None
depends_on = None
...
classified AS (
    SELECT *,
        CASE
            ...
            -- Requires sold_30d_direct >= 3 — fewer than 3 direct sales makes the average
            -- price statistically unreliable (one promotional or erroneous sale would
            -- trigger clearance for an otherwise healthy product).
            ...
            -- RUPTURE (active): has direct customer sales right now but no stock
...
_COMMENT = (
    ...
    "'schema_version=7; consumers: coverage-v3.sql, route.ts; "
