"""mv_coverage v14: 15-day window for momentum/declining detection.

The 7-day window from v13 was too noisy on low-volume SKUs (a single missed
sale collapsed momentum to 0). Doubling to 15 days smooths daily noise
while still being short enough to detect a real demand drop within 2 weeks.

Changes vs v13:
- Adds `sold_15d` (rolling 15-day sales).
- Adds `momentum_15v30` = (sold_15d/15) / daily_rate. Same semantics as
  momentum_7v30 (<0.5 collapse, >1.5 spike, NULL when no baseline).
- The ``declining`` status_base now triggers on momentum_15v30 < 0.5
  instead of momentum_7v30 < 0.5. Same threshold (0.5 = half the
  velocity of the trailing month).
...
def upgrade() -> None:
    ...
    op.execute(
        ...
        "'v14 — adds sold_15d and momentum_15v30; declining uses 15d window';"
    )


def downgrade() -> None:
    # The v13 SQL lives in 20260519_mv_coverage_v13.py — for a clean downgrade
    # the operator should re-run that migration's upgrade(). We drop here and
    # leave the upstream migration to recreate v13 on `alembic downgrade -1`.
    op.execute("DROP MATERIALIZED VIEW IF EXISTS mv_coverage;")
...
classified AS (
    SELECT *,
        -- v13 momentum_7v30 retained for backwards compat.
        CASE WHEN daily_rate > 0
            THEN ROUND((sold_7d / 7.0) / daily_rate, 2)
        END AS momentum_7v30,

        -- v14: 15-day momentum smooths out 7d noise on low-volume SKUs.
        -- Same semantics: <0.5 collapse, >1.5 spike, NULL = no baseline.
        CASE WHEN daily_rate > 0
        ...
        CASE
            ...
            -- DECLINING (v14): uses 15d window. SKU ainda vende, mas velocidade
            -- dos últimos 15 dias caiu pra metade ou menos do baseline 30d,
            -- com estoque físico para 30+ dias na taxa atual.
            WHEN sold_30d > 0
              AND (sold_15d / 15.0) < (sold_30d / 30.0) * 0.5
