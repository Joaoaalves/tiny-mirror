"""Drop mercadolivre_stock table; revert sync_type constraint.
...
the authoritative quantity from the ML API). The dedicated table and
sync_type are no longer needed.

The ``ml_oauth_tokens`` table stays — it is still required for ML auth.
