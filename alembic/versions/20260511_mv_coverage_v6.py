"""Upgrade mv_coverage to schema version 6 — active rupture detection.

Changes vs v5:
- New 'rupture (active)' rule: stock_total = 0 AND sold_30d_direct > 0
    Products that still have direct customer sales but have run out of physical
    stock. Previously fell through to 'pending' because all rupture rules
    required sold_30d = 0 (historical stockout pattern). Using sold_30d_direct
    (not sold_30d) excludes pure kit-component sales — a component with stock=0
    is not ruptured if it only moves as part of a kit whose own stock is tracked.
- sold_30d_direct exposed in metrics CTE and SELECT so the new rule can use it.

Revision ID: mv_coverage_v6
Revises: mv_coverage_v5
...
revision = "mv_coverage_v6"
down_revision = "mv_coverage_v5"
branch_labels = None
depends_on = None
...
classified AS (
    SELECT *,
        CASE
            ...
            -- Uses sold_30d_direct (not sold_30d) so kit-component expansion sales
            -- don't misclassify components whose stock is tracked by the kit itself
...
_COMMENT = (
    ...
    "'schema_version=6; consumers: coverage-v3.sql, route.ts; "
