"""mv_coverage v15: declining via 30v60 instead of 15v30.

The operator prefers a single time-window comparison across all analyses:
sold_30d vs sold_30_60d (current month vs previous month). The 15-day
window introduced in v14 adds noise without a clear payoff when the
catalog has hundreds of SKUs to scan daily.

Changes vs v14:
- `declining` status_base now triggers on sold_30d < sold_30_60d * 0.5
  (i.e., sales halved month-over-month) instead of the 15v30 momentum
  threshold. Same supplementary constraints: physical_stock covers > 1
  month and sold_30_60d >= 5 to filter out tiny-volume noise.
- `sold_15d` and `momentum_15v30` columns retained for backwards compat
  but no consumer uses them anymore. Drop in a later cleanup pass.

Revises: mv_coverage_v14
"""
...
revision = "mv_coverage_v15"
down_revision = "mv_coverage_v14"
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    ...
    op.execute(
        ...
        "'v15 — declining uses 30v60 sales ratio (15v30 removed from logic)';"
    )
...
classified AS (
    SELECT *,
        ...
        CASE
            ...
            -- DECLINING (v15): vendas caíram >= 50% mês a mês com estoque
            -- para >= 30 dias na taxa atual. Sinal pra revisar antes que vire
            -- queima formal.
            WHEN sold_30d > 0
              AND sold_30_60d >= 5
